...
## Overview
This skill covers working with the **HubSpot** integration over the Model Context Protocol (MCP) to manage CRM contacts, deals, and companies.

## When to use this skill
Activate when the user wants to manage CRM contacts, deals, and companies using HubSpot, or asks to connect, automate, or query HubSpot.

## Workflow
1. **Discover** the available HubSpot tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("hubspot", "<tool_name>", { }).results[0]
