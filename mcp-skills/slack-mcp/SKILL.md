...
## Overview
This skill covers working with the **Slack** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Slack messaging workspace operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Slack messaging workspace using the Gumloop MCP connection to `slack`.
...
current = safe_call(client, 'slack', 'list_channels', 'list_channels', {})
...
result = safe_call(client, 'slack', 'list_channels', 'list_channels', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'slack', 'send_message', 'send_message', {'channel': 'C12345', 'text': 'Hello from MCPClient!'})
...
result = safe_call(client, 'slack', 'update_message', 'update_message', {'channel': 'C12345', 'ts': '1234567890.123', 'text': 'Updated message'})
...
result = safe_call(client, 'slack', 'delete_message', 'delete_message', {'channel': 'C12345', 'ts': '1234567890.123'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `slack` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://api.slack.com/methods
- Tool discovery: Use `safe_call(client, 'slack', 'tool_discovery', {})` to list available tools at runtime.
