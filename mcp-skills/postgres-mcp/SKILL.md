...
## Overview
This skill covers working with the **Postgres** integration over the Model Context Protocol (MCP) to run read-only SQL queries against PostgreSQL.

## When to use this skill
Activate when the user wants to run read-only SQL queries against PostgreSQL using Postgres, or asks to connect, automate, or query Postgres.

## Workflow
1. **Discover** the available Postgres tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("postgres", "<tool_name>", { }).results[0]
