...
color: Blue
---
# Gdocs MCP Integration

## Overview
This skill covers working with the **Gdocs** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google Docs documents operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google Docs documents using the Gumloop MCP connection to `gdocs`.
...
current = safe_call(client, 'gdocs', 'get_document', 'get_document', {'document_id': 'doc_abc123'})
...
result = safe_call(client, 'gdocs', 'get_document', 'get_document', {'document_id': 'doc_abc123'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gdocs', 'create_document', 'create_document', {'title': 'Meeting Notes', 'body': "Notes from today's standup"})
...
result = safe_call(client, 'gdocs', 'update_document', 'update_document', {'document_id': 'doc_abc123', 'requests': [{'insertText': {'location': {'index': 1}, 'text': 'Updated: '}}]})
...
result = safe_call(client, 'gdocs', 'move_to_trash', 'move_to_trash', {'document_id': 'doc_xyz789'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gdocs` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/docs/api/
- Tool discovery: Use `safe_call(client, 'gdocs', 'tool_discovery', {})` to list available tools at runtime.
