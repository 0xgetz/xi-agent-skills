...
## Overview
This skill covers working with the **Quickbooks** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all QuickBooks accounting operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage QuickBooks accounting using the Gumloop MCP connection to `quickbooks`.
...
current = safe_call(client, 'quickbooks', 'get_invoice', 'get_invoice', {'invoice_id': 123})
...
result = safe_call(client, 'quickbooks', 'get_invoice', 'get_invoice', {'invoice_id': 123})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'quickbooks', 'create_invoice', 'create_invoice', {'customer_ref': {'value': 42}, 'line_items': [{'description': 'Consulting', 'amount': 500.00, 'detail_type': 'SalesItemLineDetail'}]})
...
result = safe_call(client, 'quickbooks', 'update_invoice', 'update_invoice', {'invoice_id': 123, 'balance': 0, 'status': 'Paid'})
...
result = safe_call(client, 'quickbooks', 'delete_invoice', 'delete_invoice', {'invoice_id': 999})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `quickbooks` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.intuit.com/app/developer/qbo/docs/api
- Tool discovery: Use `safe_call(client, 'quickbooks', 'tool_discovery', {})` to list available tools at runtime.
