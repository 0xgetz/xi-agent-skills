...
## Overview
This skill covers working with the **Salesforce** integration over the Model Context Protocol (MCP) to query and update Salesforce records.

## When to use this skill
Activate when the user wants to query and update Salesforce records using Salesforce, or asks to connect, automate, or query Salesforce.

## Workflow
1. **Discover** the available Salesforce tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("salesforce", "<tool_name>", { }).results[0]
