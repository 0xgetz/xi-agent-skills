---
name: instagram-mcp
description: Use the Instagram MCP integration to read media and account insights. Activate when the user wants to read media and account insights via Instagram, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Instagram** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Instagram content and analytics operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Instagram content and analytics using the Gumloop MCP connection to `instagram`.
...
current = safe_call(client, 'instagram', 'get_user_media', 'get_user_media', {'user_id': 'user_abc', 'limit': 10})
...
result = safe_call(client, 'instagram', 'get_user_media', 'get_user_media', {'user_id': 'user_abc', 'limit': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'instagram', 'create_media', 'create_media', {'media_type': 'IMAGE', 'image_url': 'https://example.com/photo.jpg', 'caption': 'Check out our new product!'})
...
result = safe_call(client, 'instagram', 'update_media', 'update_media', {'media_id': 'media_abc', 'caption': 'Updated: Check out our new product launch!'})
...
result = safe_call(client, 'instagram', 'delete_media', 'delete_media', {'media_id': 'media_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `instagram` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.facebook.com/docs/instagram-api/
- Tool discovery: Use `safe_call(client, 'instagram', 'tool_discovery', {})` to list available tools at runtime.
