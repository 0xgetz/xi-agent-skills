...
## Overview
This skill covers working with the **PayPal** integration over the Model Context Protocol (MCP) to process and look up PayPal transactions.

## When to use this skill
Activate when the user wants to process and look up PayPal transactions using PayPal, or asks to connect, automate, or query PayPal.

## Workflow
1. **Discover** the available PayPal tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("paypal", "<tool_name>", { }).results[0]
