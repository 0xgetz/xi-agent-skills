---
name: discord-mcp
description: Use the Discord MCP integration to send and read messages across Discord servers. Activate when the user wants to send and read messages across Discord servers via Discord, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Discord** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Discord chat platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Discord chat platform using the Gumloop MCP connection to `discord`.
...
current = safe_call(client, 'discord', 'list_channels', 'list_channels', {'guild_id': '123456'})
...
result = safe_call(client, 'discord', 'list_channels', 'list_channels', {'guild_id': '123456'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'discord', 'send_message', 'send_message', {'channel_id': '789012', 'content': 'Hello from Discord MCP!'})
...
result = safe_call(client, 'discord', 'edit_message', 'edit_message', {'channel_id': '789012', 'message_id': '345678', 'content': 'Edited message'})
...
result = safe_call(client, 'discord', 'delete_message', 'delete_message', {'channel_id': '789012', 'message_id': '345678'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `discord` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://discord.com/developers/docs
- Tool discovery: Use `safe_call(client, 'discord', 'tool_discovery', {})` to list available tools at runtime.
