...
description: Use the OpenWeather MCP integration to fetch weather forecasts and conditions. Activate when the user wants to fetch weather forecasts and conditions via OpenWeather, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Openweather** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all OpenWeather weather data operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage OpenWeather weather data using the Gumloop MCP connection to `openweather`.
...
current = safe_call(client, 'openweather', 'get_current_weather', 'get_current_weather', {'city': 'London', 'units': 'metric'})
...
result = safe_call(client, 'openweather', 'get_current_weather', 'get_current_weather', {'city': 'London', 'units': 'metric'})
print('Result:', result)
```
...
`openweather` does not have a native Gumloop MCP connector. Use raw `requests` + `os.environ` with a bound secret for write operations:
```python
import os
import requests
...
- Native API reference: https://openweathermap.org/api
- Tool discovery: Use `safe_call(client, 'openweather', 'tool_discovery', {})` to list available tools at runtime.
