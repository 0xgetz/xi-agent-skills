...
## Overview
This skill covers working with the **Gmail** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Gmail email service operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Gmail email service using the Gumloop MCP connection to `gmail`.
...
current = safe_call(client, 'gmail', 'list_emails', 'list_emails', {'max_results': 10, 'query': 'is:unread'})
...
result = safe_call(client, 'gmail', 'list_emails', 'list_emails', {'max_results': 10, 'query': 'is:unread'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gmail', 'send_email', 'send_email', {'to': ['user@example.com'], 'subject': 'Hello', 'body': 'Email body text'})
...
result = safe_call(client, 'gmail', 'modify_email', 'modify_email', {'email_id': 'abc123', 'add_labels': ['IMPORTANT'], 'remove_labels': ['INBOX']})
...
result = safe_call(client, 'gmail', 'trash_email', 'trash_email', {'email_id': 'abc123'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gmail` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/gmail/api/
- Tool discovery: Use `safe_call(client, 'gmail', 'tool_discovery', {})` to list available tools at runtime.
