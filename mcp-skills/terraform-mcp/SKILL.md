---
name: terraform-mcp
description: Use the Terraform MCP integration to plan and inspect infrastructure as code. Activate when the user wants to plan and inspect infrastructure as code via Terraform, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Terraform** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Terraform infrastructure-as-code operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Terraform infrastructure-as-code using the Gumloop MCP connection to `terraform`.
...
current = safe_call(client, 'terraform', 'list_workspaces', 'list_workspaces', {})
...
result = safe_call(client, 'terraform', 'list_workspaces', 'list_workspaces', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'terraform', 'apply_plan', 'apply_plan', {'workspace': 'production', 'plan_file': 'plan.tfplan'})
...
result = safe_call(client, 'terraform', 'set_variable', 'set_variable', {'workspace': 'staging', 'key': 'instance_count', 'value': '3'})
...
result = safe_call(client, 'terraform', 'destroy', 'destroy', {'workspace': 'staging'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `terraform` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.hashicorp.com/terraform/docs
- Tool discovery: Use `safe_call(client, 'terraform', 'tool_discovery', {})` to list available tools at runtime.
