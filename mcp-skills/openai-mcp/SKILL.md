...
## Overview
This skill covers working with the **Openai** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all OpenAI language model API operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage OpenAI language model API using the Gumloop MCP connection to `openai`.
...
current = safe_call(client, 'openai', 'list_models', 'list_models', {})
...
result = safe_call(client, 'openai', 'list_models', 'list_models', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'openai', 'create_completion', 'create_completion', {'model': 'gpt-4', 'messages': [{'role': 'user', 'content': 'Explain quantum computing'}], 'temperature': 0.7})
...
result = safe_call(client, 'openai', 'create_completion', 'create_completion', {'model': 'gpt-4', 'messages': [{'role': 'user', 'content': 'Summarize this article'}], 'max_tokens': 200})
...
result = safe_call(client, 'openai', 'delete_fine_tune_model', 'delete_fine_tune_model', {'model': 'ft:gpt-3.5-turbo:org:custom:id'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `openai` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://platform.openai.com/docs/
- Tool discovery: Use `safe_call(client, 'openai', 'tool_discovery', {})` to list available tools at runtime.
