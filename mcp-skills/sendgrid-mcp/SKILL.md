...
## Overview
This skill covers working with the **SendGrid** integration over the Model Context Protocol (MCP) to send transactional email.

## When to use this skill
Activate when the user wants to send transactional email using SendGrid, or asks to connect, automate, or query SendGrid.

## Workflow
1. **Discover** the available SendGrid tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("sendgrid", "<tool_name>", { }).results[0]
