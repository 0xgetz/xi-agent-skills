...
## Overview
This skill covers working with the **Firebase** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google Firebase services operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google Firebase services using the Gumloop MCP connection to `firebase`.
...
current = safe_call(client, 'firebase', 'get_document', 'get_document', {'collection': 'users', 'document_id': 'user_123'})
...
result = safe_call(client, 'firebase', 'get_document', 'get_document', {'collection': 'users', 'document_id': 'user_123'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'firebase', 'set_document', 'set_document', {'collection': 'users', 'document_id': 'user_123', 'data': {'name': 'Eve', 'email': 'eve@example.com'}})
...
result = safe_call(client, 'firebase', 'update_document', 'update_document', {'collection': 'users', 'document_id': 'user_123', 'data': {'last_login': '2025-01-15'}})
...
result = safe_call(client, 'firebase', 'delete_document', 'delete_document', {'collection': 'temporary', 'document_id': 'session_456'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `firebase` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://firebase.google.com/docs
- Tool discovery: Use `safe_call(client, 'firebase', 'tool_discovery', {})` to list available tools at runtime.
