---
name: onedrive-mcp
description: Use the OneDrive MCP integration to manage OneDrive files. Activate when the user wants to manage OneDrive files via OneDrive, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Onedrive** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Microsoft OneDrive personal storage operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Microsoft OneDrive personal storage using the Gumloop MCP connection to `onedrive`.
...
current = safe_call(client, 'onedrive', 'list_children', 'list_children', {'item_id': 'root'})
...
result = safe_call(client, 'onedrive', 'list_children', 'list_children', {'item_id': 'root'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'onedrive', 'upload_file', 'upload_file', {'parent_id': 'root', 'name': 'notes.txt', 'content': 'Meeting notes'})
...
result = safe_call(client, 'onedrive', 'update_item', 'update_item', {'item_id': 'item_abc', 'name': 'renamed_notes.txt'})
...
result = safe_call(client, 'onedrive', 'delete_item', 'delete_item', {'item_id': 'item_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `onedrive` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://learn.microsoft.com/en-us/onedrive/developer/
- Tool discovery: Use `safe_call(client, 'onedrive', 'tool_discovery', {})` to list available tools at runtime.
