...
## Overview
This skill covers working with the **Time** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Time and date utilities operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Time and date utilities using the Gumloop MCP connection to `time`.
...
current = safe_call(client, 'time', 'get_current_time', 'get_current_time', {'timezone': 'America/New_York'})
...
result = safe_call(client, 'time', 'get_current_time', 'get_current_time', {'timezone': 'America/New_York'})
print('Result:', result)
```
...
If the native Gumloop `time` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Tool discovery: Use `safe_call(client, 'time', 'tool_discovery', {})` to list available tools at runtime.
