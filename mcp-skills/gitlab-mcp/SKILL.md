---
name: gitlab-mcp
description: Use the GitLab MCP integration to manage GitLab projects, merge requests, and pipelines. Activate when the user wants to manage GitLab projects, merge requests, and pipelines via GitLab, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Gitlab** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all GitLab DevOps platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage GitLab DevOps platform using the Gumloop MCP connection to `gitlab`.
...
current = safe_call(client, 'gitlab', 'get_project', 'get_project', {'project_id': 12345})
...
result = safe_call(client, 'gitlab', 'get_project', 'get_project', {'project_id': 12345})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gitlab', 'create_issue', 'create_issue', {'project_id': 12345, 'title': 'Implement feature X', 'description': 'Details here', 'labels': 'enhancement'})
...
result = safe_call(client, 'gitlab', 'update_issue', 'update_issue', {'project_id': 12345, 'issue_iid': 42, 'state_event': 'close'})
...
result = safe_call(client, 'gitlab', 'delete_project', 'delete_project', {'project_id': 99999})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gitlab` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.gitlab.com/ee/api/
- Tool discovery: Use `safe_call(client, 'gitlab', 'tool_discovery', {})` to list available tools at runtime.
