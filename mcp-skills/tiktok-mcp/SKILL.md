...
## Overview
This skill covers working with the **Tiktok** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all TikTok video platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage TikTok video platform using the Gumloop MCP connection to `tiktok`.
...
current = safe_call(client, 'tiktok', 'get_user_info', 'get_user_info', {'username': '@official'})
...
result = safe_call(client, 'tiktok', 'get_user_info', 'get_user_info', {'username': '@official'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'tiktok', 'upload_video', 'upload_video', {'video_url': 'https://example.com/video.mp4', 'caption': 'New dance trend!', 'privacy_level': 'PUBLIC'})
...
result = safe_call(client, 'tiktok', 'update_video', 'update_video', {'video_id': 'vid_abc', 'caption': 'Updated: This went viral!'})
...
result = safe_call(client, 'tiktok', 'delete_video', 'delete_video', {'video_id': 'vid_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `tiktok` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.tiktok.com/
- Tool discovery: Use `safe_call(client, 'tiktok', 'tool_discovery', {})` to list available tools at runtime.
