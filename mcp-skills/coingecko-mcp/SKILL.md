...
## Overview
This skill covers working with the **Coingecko** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all CoinGecko cryptocurrency data operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage CoinGecko cryptocurrency data using the Gumloop MCP connection to `coingecko`.
...
current = safe_call(client, 'coingecko', 'get_price', 'get_price', {'ids': ['bitcoin', 'ethereum'], 'vs_currencies': ['usd']})
...
result = safe_call(client, 'coingecko', 'get_price', 'get_price', {'ids': ['bitcoin', 'ethereum'], 'vs_currencies': ['usd']})
print('Result:', result)
```
...
`coingecko` does not have a native Gumloop MCP connector. Use raw `requests` + `os.environ` with a bound secret for write operations:
```python
import os
import requests
...
- Native API reference: https://www.coingecko.com/en/api/documentation
- Tool discovery: Use `safe_call(client, 'coingecko', 'tool_discovery', {})` to list available tools at runtime.
