---
name: memory-mcp
description: Use the Memory MCP integration to store and recall persistent knowledge graph memory. Activate when the user wants to store and recall persistent knowledge graph memory via Memory, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Memory** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Persistent memory storage operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Persistent memory storage using the Gumloop MCP connection to `memory`.
...
current = safe_call(client, 'memory', 'get_memories', 'get_memories', {'query': 'user preferences', 'limit': 5})
...
result = safe_call(client, 'memory', 'get_memories', 'get_memories', {'query': 'user preferences', 'limit': 5})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'memory', 'store_memory', 'store_memory', {'key': 'user_preference_theme', 'value': 'dark', 'namespace': 'user_123'})
...
result = safe_call(client, 'memory', 'update_memory', 'update_memory', {'key': 'user_preference_theme', 'value': 'light'})
...
result = safe_call(client, 'memory', 'delete_memory', 'delete_memory', {'key': 'temporary_note'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `memory` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: N/A (built-in memory utility)
- Tool discovery: Use `safe_call(client, 'memory', 'tool_discovery', {})` to list available tools at runtime.
