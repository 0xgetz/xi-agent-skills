...
If the native Gumloop `github` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.github.com/en/rest
- Tool discovery: Use `safe_call(client, 'github', 'tool_discovery', {})` to list available tools at runtime.
