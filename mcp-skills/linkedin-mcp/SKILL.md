---
name: linkedin-mcp
description: Use the LinkedIn MCP integration to read profiles and post updates. Activate when the user wants to read profiles and post updates via LinkedIn, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Linkedin** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all LinkedIn professional network operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage LinkedIn professional network using the Gumloop MCP connection to `linkedin`.
...
current = safe_call(client, 'linkedin', 'get_profile', 'get_profile', {'profile_id': 'urn:li:person:abc'})
...
result = safe_call(client, 'linkedin', 'get_profile', 'get_profile', {'profile_id': 'urn:li:person:abc'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'linkedin', 'create_post', 'create_post', {'author': 'urn:li:organization:xyz', 'text': 'We are hiring! Check out our open positions.'})
...
result = safe_call(client, 'linkedin', 'update_post', 'update_post', {'post_id': 'post_abc', 'text': 'Updated: Positions now open!'})
...
result = safe_call(client, 'linkedin', 'delete_post', 'delete_post', {'post_id': 'post_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `linkedin` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://learn.microsoft.com/en-us/linkedin/
- Tool discovery: Use `safe_call(client, 'linkedin', 'tool_discovery', {})` to list available tools at runtime.
