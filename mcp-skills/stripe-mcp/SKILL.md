...
## Overview
This skill covers working with the **Stripe** integration over the Model Context Protocol (MCP) to manage customers, charges, and subscriptions.

## When to use this skill
Activate when the user wants to manage customers, charges, and subscriptions using Stripe, or asks to connect, automate, or query Stripe.

## Workflow
1. **Discover** the available Stripe tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("stripe", "<tool_name>", { }).results[0]
