...
color: Blue
---
# Aws_Lambda MCP Integration

## Overview
This skill covers working with the **Aws_Lambda** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all AWS Lambda serverless functions operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage AWS Lambda serverless functions using the Gumloop MCP connection to `aws_lambda`.
...
current = safe_call(client, 'aws_lambda', 'list_functions', 'list_functions', {'max_items': 20})
...
result = safe_call(client, 'aws_lambda', 'list_functions', 'list_functions', {'max_items': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'aws_lambda', 'invoke', 'invoke', {'function_name': 'my-function', 'payload': {'event': 'data'}})
...
result = safe_call(client, 'aws_lambda', 'update_function_code', 'update_function_code', {'function_name': 'my-function', 's3_key': 'code/v2.zip'})
...
result = safe_call(client, 'aws_lambda', 'delete_function', 'delete_function', {'function_name': 'old-function'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `aws_lambda` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.aws.amazon.com/lambda/
- Tool discovery: Use `safe_call(client, 'aws_lambda', 'tool_discovery', {})` to list available tools at runtime.
