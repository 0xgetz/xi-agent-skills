...
description: Use the Google Sheets MCP integration to read and write spreadsheet data. Activate when the user wants to read and write spreadsheet data via Google Sheets, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
# Gsheets MCP Integration

## Overview
This skill covers working with the **Gsheets** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google Sheets spreadsheets operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google Sheets spreadsheets using the Gumloop MCP connection to `gsheets`.
...
current = safe_call(client, 'gsheets', 'get_values', 'get_values', {'spreadsheet_id': 'sheet_abc', 'range': 'Sheet1!A1:D10'})
...
result = safe_call(client, 'gsheets', 'get_values', 'get_values', {'spreadsheet_id': 'sheet_abc', 'range': 'Sheet1!A1:D10'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gsheets', 'update_values', 'update_values', {'spreadsheet_id': 'sheet_abc', 'range': 'Sheet1!A1', 'values': [['Name', 'Email', 'Status'], ['Alice', 'alice@test.com', 'Active']]})
...
result = safe_call(client, 'gsheets', 'append_values', 'append_values', {'spreadsheet_id': 'sheet_abc', 'range': 'Sheet1!A:D', 'values': [['Bob', 'bob@test.com', 'Pending']], 'value_input_option': 'USER_ENTERED'})
...
result = safe_call(client, 'gsheets', 'clear_values', 'clear_values', {'spreadsheet_id': 'sheet_abc', 'range': 'Sheet1!A2:D100'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gsheets` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/sheets/api/
- Tool discovery: Use `safe_call(client, 'gsheets', 'tool_discovery', {})` to list available tools at runtime.
