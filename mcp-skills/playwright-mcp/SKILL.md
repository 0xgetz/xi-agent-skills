...
## Overview
This skill covers working with the **Playwright** integration over the Model Context Protocol (MCP) to drive browsers for end-to-end automation.

## When to use this skill
Activate when the user wants to drive browsers for end-to-end automation using Playwright, or asks to connect, automate, or query Playwright.

## Workflow
1. **Discover** the available Playwright tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("playwright", "<tool_name>", { }).results[0]
