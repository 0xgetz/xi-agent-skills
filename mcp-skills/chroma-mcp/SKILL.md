...
## Overview
This skill covers working with the **Chroma** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all ChromaDB vector store operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage ChromaDB vector store using the Gumloop MCP connection to `chroma`.
...
current = safe_call(client, 'chroma', 'query', 'query', {'collection': 'embeddings', 'query_texts': ['machine learning'], 'n_results': 5})
...
result = safe_call(client, 'chroma', 'query', 'query', {'collection': 'embeddings', 'query_texts': ['machine learning'], 'n_results': 5})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'chroma', 'add_documents', 'add_documents', {'collection': 'embeddings', 'documents': ['AI is transforming industry'], 'metadatas': [{'source': 'blog'}], 'ids': ['doc1']})
...
result = safe_call(client, 'chroma', 'update_documents', 'update_documents', {'collection': 'embeddings', 'ids': ['doc1'], 'metadatas': [{'source': 'updated'}]})
...
result = safe_call(client, 'chroma', 'delete', 'delete', {'collection': 'embeddings', 'ids': ['doc1']})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `chroma` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.trychroma.com/
- Tool discovery: Use `safe_call(client, 'chroma', 'tool_discovery', {})` to list available tools at runtime.
