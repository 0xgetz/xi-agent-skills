...
## Overview
This skill covers working with the **Anthropic** integration over the Model Context Protocol (MCP) to call Claude models for completions.

## When to use this skill
Activate when the user wants to call Claude models for completions using Anthropic, or asks to connect, automate, or query Anthropic.

## Workflow
1. **Discover** the available Anthropic tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("anthropic", "<tool_name>", { }).results[0]
