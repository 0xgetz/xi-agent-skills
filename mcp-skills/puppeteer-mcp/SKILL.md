...
## Overview
This skill covers working with the **Puppeteer** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Puppeteer browser automation operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Puppeteer browser automation using the Gumloop MCP connection to `puppeteer`.
...
current = safe_call(client, 'puppeteer', 'navigate', 'navigate', {'url': 'https://example.com'})
...
result = safe_call(client, 'puppeteer', 'navigate', 'navigate', {'url': 'https://example.com'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'puppeteer', 'click', 'click', {'selector': '#submit-button'})
...
result = safe_call(client, 'puppeteer', 'type', 'type', {'selector': '#search-input', 'text': 'MCP integration'})
...
result = safe_call(client, 'puppeteer', 'close', 'close', {})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `puppeteer` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://pptr.dev/
- Tool discovery: Use `safe_call(client, 'puppeteer', 'tool_discovery', {})` to list available tools at runtime.
