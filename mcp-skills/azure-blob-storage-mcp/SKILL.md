...
description: Use the Azure Blob Storage MCP integration to manage Azure blob containers. Activate when the user wants to manage Azure blob containers via Azure Blob Storage, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
# Azure_Blob MCP Integration

## Overview
This skill covers working with the **Azure_Blob** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Azure Blob Storage operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Azure Blob Storage using the Gumloop MCP connection to `azure_blob`.
...
current = safe_call(client, 'azure_blob', 'list_blobs', 'list_blobs', {'container': 'data', 'prefix': '2025/'})
...
result = safe_call(client, 'azure_blob', 'list_blobs', 'list_blobs', {'container': 'data', 'prefix': '2025/'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'azure_blob', 'upload_blob', 'upload_blob', {'container': 'data', 'blob_name': '2025/report.json', 'data': 'content here'})
...
result = safe_call(client, 'azure_blob', 'set_blob_properties', 'set_blob_properties', {'container': 'data', 'blob_name': 'report.json', 'properties': {'content_type': 'application/json'}})
...
result = safe_call(client, 'azure_blob', 'delete_blob', 'delete_blob', {'container': 'temp', 'blob_name': 'old_backup.bak'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `azure_blob` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://learn.microsoft.com/en-us/azure/storage/blobs/
- Tool discovery: Use `safe_call(client, 'azure_blob', 'tool_discovery', {})` to list available tools at runtime.
