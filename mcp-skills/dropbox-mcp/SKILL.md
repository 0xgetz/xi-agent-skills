...
## Overview
This skill covers working with the **Dropbox** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Dropbox cloud storage operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Dropbox cloud storage using the Gumloop MCP connection to `dropbox`.
...
current = safe_call(client, 'dropbox', 'list_folder', 'list_folder', {'path': '/Documents', 'limit': 20})
...
result = safe_call(client, 'dropbox', 'list_folder', 'list_folder', {'path': '/Documents', 'limit': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'dropbox', 'upload_file', 'upload_file', {'path': '/Documents/report.pdf', 'content': '...', 'mode': 'add'})
...
result = safe_call(client, 'dropbox', 'move_file', 'move_file', {'from_path': '/temp/file.txt', 'to_path': '/Documents/file.txt'})
...
result = safe_call(client, 'dropbox', 'delete_file', 'delete_file', {'path': '/temp/old_file.txt'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `dropbox` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://www.dropbox.com/developers/documentation/http/
- Tool discovery: Use `safe_call(client, 'dropbox', 'tool_discovery', {})` to list available tools at runtime.
