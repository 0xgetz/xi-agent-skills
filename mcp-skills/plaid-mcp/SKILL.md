...
## Overview
This skill covers working with the **Plaid** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Plaid financial data aggregation operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Plaid financial data aggregation using the Gumloop MCP connection to `plaid`.
...
current = safe_call(client, 'plaid', 'get_accounts', 'get_accounts', {'access_token': 'access-sandbox-abc'})
...
result = safe_call(client, 'plaid', 'get_accounts', 'get_accounts', {'access_token': 'access-sandbox-abc'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'plaid', 'create_link_token', 'create_link_token', {'user': {'client_user_id': 'user_123'}, 'products': ['transactions'], 'country_codes': ['US']})
...
result = safe_call(client, 'plaid', 'update_item_webhook', 'update_item_webhook', {'access_token': 'access-sandbox-abc', 'webhook': 'https://example.com/plaid-webhook'})
...
result = safe_call(client, 'plaid', 'remove_item', 'remove_item', {'access_token': 'access-sandbox-xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `plaid` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://plaid.com/docs/api/
- Tool discovery: Use `safe_call(client, 'plaid', 'tool_discovery', {})` to list available tools at runtime.
