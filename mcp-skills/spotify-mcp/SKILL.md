...
## Overview
This skill covers working with the **Spotify** integration over the Model Context Protocol (MCP) to search music and manage playlists.

## When to use this skill
Activate when the user wants to search music and manage playlists using Spotify, or asks to connect, automate, or query Spotify.

## Workflow
1. **Discover** the available Spotify tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("spotify", "<tool_name>", { }).results[0]
