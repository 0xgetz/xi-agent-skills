---
name: apify-mcp
description: Use the Apify MCP integration to run web scraping and automation actors. Activate when the user wants to run web scraping and automation actors via Apify, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Apify** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Apify web scraping platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Apify web scraping platform using the Gumloop MCP connection to `apify`.
...
current = safe_call(client, 'apify', 'get_dataset_items', 'get_dataset_items', {'dataset_id': 'ds_abc', 'limit': 10})
...
result = safe_call(client, 'apify', 'get_dataset_items', 'get_dataset_items', {'dataset_id': 'ds_abc', 'limit': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'apify', 'run_actor', 'run_actor', {'actor_id': 'apify/web-scraper', 'input': {'startUrls': [{'url': 'https://example.com'}], 'maxPagesPerCrawl': 10}})
...
result = safe_call(client, 'apify', 'run_actor', 'run_actor', {'actor_id': 'apify/web-scraper', 'input': {'startUrls': [{'url': 'https://example.com'}], 'maxPagesPerCrawl': 20}})
...
result = safe_call(client, 'apify', 'delete_dataset', 'delete_dataset', {'dataset_id': 'ds_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `apify` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.apify.com/api/
- Tool discovery: Use `safe_call(client, 'apify', 'tool_discovery', {})` to list available tools at runtime.
