...
# Microsoft Excel MCP Integration

## Overview
This skill covers working with the **Microsoft Excel** integration over the Model Context Protocol (MCP) to read and write Excel workbooks.

## When to use this skill
Activate when the user wants to read and write Excel workbooks using Microsoft Excel, or asks to connect, automate, or query Microsoft Excel.

## Workflow
1. **Discover** the available Microsoft Excel tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("microsoft-excel", "<tool_name>", { }).results[0]
