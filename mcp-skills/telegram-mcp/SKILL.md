...
## Overview
This skill covers working with the **Telegram** integration over the Model Context Protocol (MCP) to send and read Telegram messages.

## When to use this skill
Activate when the user wants to send and read Telegram messages using Telegram, or asks to connect, automate, or query Telegram.

## Workflow
1. **Discover** the available Telegram tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("telegram", "<tool_name>", { }).results[0]
