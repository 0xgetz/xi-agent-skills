...
## Overview
This skill covers working with the **Prometheus** integration over the Model Context Protocol (MCP) to query time-series metrics.

## When to use this skill
Activate when the user wants to query time-series metrics using Prometheus, or asks to connect, automate, or query Prometheus.

## Workflow
1. **Discover** the available Prometheus tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("prometheus", "<tool_name>", { }).results[0]
