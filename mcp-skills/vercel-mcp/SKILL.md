---
name: vercel-mcp
description: Use the Vercel MCP integration to manage deployments and projects. Activate when the user wants to manage deployments and projects via Vercel, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Vercel** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Vercel hosting platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Vercel hosting platform using the Gumloop MCP connection to `vercel`.
...
current = safe_call(client, 'vercel', 'list_projects', 'list_projects', {})
...
result = safe_call(client, 'vercel', 'list_projects', 'list_projects', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'vercel', 'deploy', 'deploy', {'name': 'my-app', 'files': {'index.html': '<h1>Hello</h1>'}})
...
result = safe_call(client, 'vercel', 'update_project', 'update_project', {'project_id': 'prj_abc', 'framework': 'nextjs'})
...
result = safe_call(client, 'vercel', 'delete_project', 'delete_project', {'project_id': 'prj_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `vercel` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://vercel.com/docs
- Tool discovery: Use `safe_call(client, 'vercel', 'tool_discovery', {})` to list available tools at runtime.
