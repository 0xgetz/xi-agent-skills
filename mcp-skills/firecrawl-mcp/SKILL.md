---
name: firecrawl-mcp
description: Use the Firecrawl MCP integration to crawl and scrape websites into structured data. Activate when the user wants to crawl and scrape websites into structured data via Firecrawl, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Firecrawl** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Firecrawl web scraping operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Firecrawl web scraping using the Gumloop MCP connection to `firecrawl`.
...
current = safe_call(client, 'firecrawl', 'scrape_url', 'scrape_url', {'url': 'https://example.com', 'formats': ['markdown', 'metadata']})
...
result = safe_call(client, 'firecrawl', 'scrape_url', 'scrape_url', {'url': 'https://example.com', 'formats': ['markdown', 'metadata']})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'firecrawl', 'crawl_url', 'crawl_url', {'url': 'https://docs.example.com', 'max_pages': 50, 'include_paths': ['/docs/*']})
...
result = safe_call(client, 'firecrawl', 'crawl_url', 'crawl_url', {'url': 'https://example.com', 'max_pages': 10, 'max_depth': 2})
...
result = safe_call(client, 'firecrawl', 'cancel_crawl', 'cancel_crawl', {'crawl_id': 'crawl_abc123'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `firecrawl` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.firecrawl.dev/
- Tool discovery: Use `safe_call(client, 'firecrawl', 'tool_discovery', {})` to list available tools at runtime.
