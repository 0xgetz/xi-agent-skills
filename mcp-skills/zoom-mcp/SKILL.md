---
name: zoom-mcp
description: Use the Zoom MCP integration to manage meetings and recordings. Activate when the user wants to manage meetings and recordings via Zoom, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Zoom** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Zoom video conferencing operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Zoom video conferencing using the Gumloop MCP connection to `zoom`.
...
current = safe_call(client, 'zoom', 'list_meetings', 'list_meetings', {'user_id': 'me'})
...
result = safe_call(client, 'zoom', 'list_meetings', 'list_meetings', {'user_id': 'me'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'zoom', 'create_meeting', 'create_meeting', {'topic': 'Sprint Review', 'duration': 30, 'type': 2})
...
result = safe_call(client, 'zoom', 'update_meeting', 'update_meeting', {'meeting_id': 123456789, 'topic': 'Updated Topic'})
...
result = safe_call(client, 'zoom', 'delete_meeting', 'delete_meeting', {'meeting_id': 123456789})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `zoom` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.zoom.us/docs/api/
- Tool discovery: Use `safe_call(client, 'zoom', 'tool_discovery', {})` to list available tools at runtime.
