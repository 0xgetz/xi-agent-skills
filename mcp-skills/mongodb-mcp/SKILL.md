---
name: mongodb-mcp
description: Use the MongoDB MCP integration to query and aggregate MongoDB collections. Activate when the user wants to query and aggregate MongoDB collections via MongoDB, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Mongodb** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all MongoDB document databases operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage MongoDB document databases using the Gumloop MCP connection to `mongodb`.
...
current = safe_call(client, 'mongodb', 'find_documents', 'find_documents', {'database': 'mydb', 'collection': 'users', 'filter': {'active': True}, 'limit': 10})
...
result = safe_call(client, 'mongodb', 'find_documents', 'find_documents', {'database': 'mydb', 'collection': 'users', 'filter': {'active': True}, 'limit': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'mongodb', 'insert_document', 'insert_document', {'database': 'mydb', 'collection': 'users', 'document': {'name': 'Alice', 'email': 'alice@example.com'}})
...
result = safe_call(client, 'mongodb', 'update_document', 'update_document', {'database': 'mydb', 'collection': 'users', 'filter': {'email': 'alice@example.com'}, 'update': {'$set': {'status': 'active'}}})
...
result = safe_call(client, 'mongodb', 'delete_documents', 'delete_documents', {'database': 'mydb', 'collection': 'users', 'filter': {'status': 'inactive'}})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `mongodb` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://www.mongodb.com/docs/manual/
- Tool discovery: Use `safe_call(client, 'mongodb', 'tool_discovery', {})` to list available tools at runtime.
