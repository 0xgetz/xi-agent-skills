---
name: docker-mcp
description: Use the Docker MCP integration to manage Docker containers and images. Activate when the user wants to manage Docker containers and images via Docker, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Docker** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Docker container management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Docker container management using the Gumloop MCP connection to `docker`.
...
current = safe_call(client, 'docker', 'list_containers', 'list_containers', {'all': False})
...
result = safe_call(client, 'docker', 'list_containers', 'list_containers', {'all': False})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'docker', 'run_container', 'run_container', {'image': 'nginx:latest', 'name': 'web-server', 'ports': {'80/tcp': 8080}})
...
result = safe_call(client, 'docker', 'restart_container', 'restart_container', {'container_id': 'abc123'})
...
result = safe_call(client, 'docker', 'remove_container', 'remove_container', {'container_id': 'old-container', 'force': True})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `docker` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.docker.com/
- Tool discovery: Use `safe_call(client, 'docker', 'tool_discovery', {})` to list available tools at runtime.
