...
## Overview
This skill covers working with the **Snowflake** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Snowflake data cloud operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Snowflake data cloud using the Gumloop MCP connection to `snowflake`.
...
current = safe_call(client, 'snowflake', 'run_query', 'run_query', {'query': 'SELECT * FROM analytics.sessions WHERE event_date > CURRENT_DATE - 7'})
...
result = safe_call(client, 'snowflake', 'run_query', 'run_query', {'query': 'SELECT * FROM analytics.sessions WHERE event_date > CURRENT_DATE - 7'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'snowflake', 'run_query', 'run_query', {'query': "INSERT INTO analytics.events (user_id, event) VALUES ('u123', 'page_view')"})
...
result = safe_call(client, 'snowflake', 'run_query', 'run_query', {'query': 'UPDATE analytics.users SET tier = "premium" WHERE id = "u123"'})
...
result = safe_call(client, 'snowflake', 'run_query', 'run_query', {'query': 'DELETE FROM analytics.staging WHERE loaded = true'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `snowflake` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.snowflake.com/en/
- Tool discovery: Use `safe_call(client, 'snowflake', 'tool_discovery', {})` to list available tools at runtime.
