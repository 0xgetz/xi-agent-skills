---
name: airtable-mcp
description: Use the Airtable MCP integration to read and write Airtable bases and tables. Activate when the user wants to read and write Airtable bases and tables via Airtable, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Airtable** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Airtable bases and tables operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Airtable bases and tables using the Gumloop MCP connection to `airtable`.
...
current = safe_call(client, 'airtable', 'list_records', 'list_records', {'base_id': 'appXXXXX', 'table_name': 'Leads'})
...
result = safe_call(client, 'airtable', 'list_records', 'list_records', {'base_id': 'appXXXXX', 'table_name': 'Leads'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'airtable', 'create_record', 'create_record', {'base_id': 'appXXXXX', 'table_name': 'Leads', 'fields': {'Name': 'Acme Corp', 'Status': 'New'}})
...
result = safe_call(client, 'airtable', 'update_record', 'update_record', {'base_id': 'appXXXXX', 'table_name': 'Leads', 'record_id': 'recXXXXX', 'fields': {'Status': 'Contacted'}})
...
result = safe_call(client, 'airtable', 'delete_record', 'delete_record', {'base_id': 'appXXXXX', 'table_name': 'Leads', 'record_id': 'recXXXXX'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `airtable` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://airtable.com/developers/web/api
- Tool discovery: Use `safe_call(client, 'airtable', 'tool_discovery', {})` to list available tools at runtime.
