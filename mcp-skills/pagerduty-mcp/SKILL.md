...
## Overview
This skill covers working with the **Pagerduty** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all PagerDuty incident management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage PagerDuty incident management using the Gumloop MCP connection to `pagerduty`.
...
current = safe_call(client, 'pagerduty', 'list_incidents', 'list_incidents', {'statuses': ['triggered', 'acknowledged']})
...
result = safe_call(client, 'pagerduty', 'list_incidents', 'list_incidents', {'statuses': ['triggered', 'acknowledged']})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'pagerduty', 'acknowledge_incident', 'acknowledge_incident', {'incident_id': 'INCIDENT_ABC'})
...
result = safe_call(client, 'pagerduty', 'update_incident', 'update_incident', {'incident_id': 'INCIDENT_ABC', 'status': 'acknowledged', 'priority': 'P1'})
...
result = safe_call(client, 'pagerduty', 'resolve_incident', 'resolve_incident', {'incident_id': 'INCIDENT_ABC'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `pagerduty` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.pagerduty.com/
- Tool discovery: Use `safe_call(client, 'pagerduty', 'tool_discovery', {})` to list available tools at runtime.
