...
## Overview
This skill covers working with the **Fetch** integration over the Model Context Protocol (MCP) to fetch and convert web page content to text.

## When to use this skill
Activate when the user wants to fetch and convert web page content to text using Fetch, or asks to connect, automate, or query Fetch.

## Workflow
1. **Discover** the available Fetch tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("fetch", "<tool_name>", { }).results[0]
