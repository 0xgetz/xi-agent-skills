...
## Overview
This skill covers working with the **Mysql** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all MySQL databases operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage MySQL databases using the Gumloop MCP connection to `mysql`.
...
current = safe_call(client, 'mysql', 'run_query', 'run_query', {'query': 'SELECT * FROM customers ORDER BY created_at DESC LIMIT 10'})
...
result = safe_call(client, 'mysql', 'run_query', 'run_query', {'query': 'SELECT * FROM customers ORDER BY created_at DESC LIMIT 10'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'mysql', 'run_query', 'run_query', {'query': "INSERT INTO customers (name, email) VALUES ('Carol', 'carol@example.com')"})
...
result = safe_call(client, 'mysql', 'run_query', 'run_query', {'query': "UPDATE customers SET status = 'vip' WHERE id = 7"})
...
result = safe_call(client, 'mysql', 'run_query', 'run_query', {'query': 'DELETE FROM customers WHERE id = 7'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `mysql` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://dev.mysql.com/doc/
- Tool discovery: Use `safe_call(client, 'mysql', 'tool_discovery', {})` to list available tools at runtime.
