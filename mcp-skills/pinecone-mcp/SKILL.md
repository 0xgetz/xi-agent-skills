...
## Overview
This skill covers working with the **Pinecone** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Pinecone vector database operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Pinecone vector database using the Gumloop MCP connection to `pinecone`.
...
current = safe_call(client, 'pinecone', 'query', 'query', {'index': 'products', 'vector': [0.1, 0.2, 0.3], 'top_k': 5})
...
result = safe_call(client, 'pinecone', 'query', 'query', {'index': 'products', 'vector': [0.1, 0.2, 0.3], 'top_k': 5})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'pinecone', 'upsert', 'upsert', {'index': 'products', 'vectors': [{'id': 'vec1', 'values': [0.1, 0.2, 0.3], 'metadata': {'name': 'Widget'}}]})
...
result = safe_call(client, 'pinecone', 'update', 'update', {'index': 'products', 'id': 'vec1', 'metadata': {'price': 29.99}})
...
result = safe_call(client, 'pinecone', 'delete', 'delete', {'index': 'products', 'ids': ['vec1', 'vec2']})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `pinecone` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.pinecone.io/
- Tool discovery: Use `safe_call(client, 'pinecone', 'tool_discovery', {})` to list available tools at runtime.
