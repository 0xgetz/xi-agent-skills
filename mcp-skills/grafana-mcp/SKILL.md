---
name: grafana-mcp
description: Use the Grafana MCP integration to query dashboards and metrics. Activate when the user wants to query dashboards and metrics via Grafana, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Grafana** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Grafana analytics dashboards operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Grafana analytics dashboards using the Gumloop MCP connection to `grafana`.
...
current = safe_call(client, 'grafana', 'search_dashboards', 'search_dashboards', {'query': 'Production'})
...
result = safe_call(client, 'grafana', 'search_dashboards', 'search_dashboards', {'query': 'Production'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'grafana', 'create_dashboard', 'create_dashboard', {'dashboard': {'title': 'API Latency', 'panels': [{'type': 'graph'}]}})
...
result = safe_call(client, 'grafana', 'update_dashboard', 'update_dashboard', {'dashboard': {'uid': 'abc123', 'title': 'Updated Dashboard'}})
...
result = safe_call(client, 'grafana', 'delete_dashboard', 'delete_dashboard', {'uid': 'abc123'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `grafana` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://grafana.com/docs/grafana/latest/developers/http_api/
- Tool discovery: Use `safe_call(client, 'grafana', 'tool_discovery', {})` to list available tools at runtime.
