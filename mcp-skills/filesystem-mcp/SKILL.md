...
## Overview
This skill covers working with the **Filesystem** integration over the Model Context Protocol (MCP) to read/write local files and directories.

## When to use this skill
Activate when the user wants to read/write local files and directories using Filesystem, or asks to connect, automate, or query Filesystem.

## Workflow
1. **Discover** the available Filesystem tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("filesystem", "<tool_name>", { }).results[0]
