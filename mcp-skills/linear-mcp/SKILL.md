---
name: linear-mcp
description: Use the Linear MCP integration to manage Linear issues and projects. Activate when the user wants to manage Linear issues and projects via Linear, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Linear** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Linear issue tracking operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Linear issue tracking using the Gumloop MCP connection to `linear`.
...
current = safe_call(client, 'linear', 'get_issue', 'get_issue', {'issue_id': 'abc-123'})
...
result = safe_call(client, 'linear', 'get_issue', 'get_issue', {'issue_id': 'abc-123'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'linear', 'create_issue', 'create_issue', {'team_id': 'team_abc', 'title': 'Implement dark mode', 'description': 'Add dark mode toggle'})
...
result = safe_call(client, 'linear', 'update_issue', 'update_issue', {'issue_id': 'abc-123', 'state_id': 'done_state', 'assignee_id': 'user_abc'})
...
result = safe_call(client, 'linear', 'delete_issue', 'delete_issue', {'issue_id': 'abc-999'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `linear` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.linear.app/docs/graphql/working-with-the-graphql-api
- Tool discovery: Use `safe_call(client, 'linear', 'tool_discovery', {})` to list available tools at runtime.
