...
## Overview
This skill covers working with the **Hugging Face** integration over the Model Context Protocol (MCP) to run inference on Hugging Face models.

## When to use this skill
Activate when the user wants to run inference on Hugging Face models using Hugging Face, or asks to connect, automate, or query Hugging Face.

## Workflow
1. **Discover** the available Hugging Face tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("hugging-face", "<tool_name>", { }).results[0]
