...
## Overview
This skill covers working with the **Polygon** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Polygon.io stock market data operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Polygon.io stock market data using the Gumloop MCP connection to `polygon`.
...
current = safe_call(client, 'polygon', 'get_aggregates', 'get_aggregates', {'ticker': 'AAPL', 'multiplier': 1, 'timespan': 'day', 'from': '2025-01-01', 'to': '2025-01-15'})
...
result = safe_call(client, 'polygon', 'get_aggregates', 'get_aggregates', {'ticker': 'AAPL', 'multiplier': 1, 'timespan': 'day', 'from': '2025-01-01', 'to': '2025-01-15'})
print('Result:', result)
```
...
`polygon` does not have a native Gumloop MCP connector. Use raw `requests` + `os.environ` with a bound secret for write operations:
```python
import os
import requests
...
- Native API reference: https://polygon.io/docs/
- Tool discovery: Use `safe_call(client, 'polygon', 'tool_discovery', {})` to list available tools at runtime.
