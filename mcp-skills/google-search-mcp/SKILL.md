...
description: Use the Google Search MCP integration to search the web and retrieve results. Activate when the user wants to search the web and retrieve results via Google Search, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
# Google_Search MCP Integration

## Overview
This skill covers working with the **Google_Search** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google Search API operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google Search API using the Gumloop MCP connection to `google_search`.
...
current = safe_call(client, 'google_search', 'search', 'search', {'query': 'latest AI research papers', 'num': 5})
...
result = safe_call(client, 'google_search', 'search', 'search', {'query': 'latest AI research papers', 'num': 5})
print('Result:', result)
```
...
`google_search` does not have a native Gumloop MCP connector. Use raw `requests` + `os.environ` with a bound secret for write operations:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/custom-search/
- Tool discovery: Use `safe_call(client, 'google_search', 'tool_discovery', {})` to list available tools at runtime.
