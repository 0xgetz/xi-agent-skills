...
## Overview
This skill covers working with the **Qdrant** integration over the Model Context Protocol (MCP) to store and search vectors in Qdrant.

## When to use this skill
Activate when the user wants to store and search vectors in Qdrant using Qdrant, or asks to connect, automate, or query Qdrant.

## Workflow
1. **Discover** the available Qdrant tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("qdrant", "<tool_name>", { }).results[0]
