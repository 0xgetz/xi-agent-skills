...
## Overview
This skill covers working with the **Jira** integration over the Model Context Protocol (MCP) to manage Jira issues and sprints.

## When to use this skill
Activate when the user wants to manage Jira issues and sprints using Jira, or asks to connect, automate, or query Jira.

## Workflow
1. **Discover** the available Jira tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("jira", "<tool_name>", { }).results[0]
