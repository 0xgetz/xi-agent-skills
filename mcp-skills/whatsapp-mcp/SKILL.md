---
name: whatsapp-mcp
description: Use the WhatsApp MCP integration to send and read WhatsApp messages. Activate when the user wants to send and read WhatsApp messages via WhatsApp, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Whatsapp** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all WhatsApp Business messaging operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage WhatsApp Business messaging using the Gumloop MCP connection to `whatsapp`.
...
current = safe_call(client, 'whatsapp', 'list_templates', 'list_templates', {})
...
result = safe_call(client, 'whatsapp', 'list_templates', 'list_templates', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'whatsapp', 'send_text', 'send_text', {'to': '+1234567890', 'message': 'Hello from WhatsApp MCP!'})
...
result = safe_call(client, 'whatsapp', 'update_template', 'update_template', {'template_name': 'welcome', 'body': 'Welcome {{1}}!'})
...
result = safe_call(client, 'whatsapp', 'delete_template', 'delete_template', {'template_name': 'old_template'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `whatsapp` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.facebook.com/docs/whatsapp/
- Tool discovery: Use `safe_call(client, 'whatsapp', 'tool_discovery', {})` to list available tools at runtime.
