...
## Overview
This skill covers working with the **Monday** integration over the Model Context Protocol (MCP) to manage monday.com boards and items.

## When to use this skill
Activate when the user wants to manage monday.com boards and items using Monday, or asks to connect, automate, or query Monday.

## Workflow
1. **Discover** the available Monday tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("monday", "<tool_name>", { }).results[0]
