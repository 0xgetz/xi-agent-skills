...
If the native Gumloop `shopify` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://shopify.dev/docs/api/admin-rest
- Tool discovery: Use `safe_call(client, 'shopify', 'tool_discovery', {})` to list available tools at runtime.
