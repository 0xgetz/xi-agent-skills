---
name: netlify-mcp
description: Use the Netlify MCP integration to manage sites and deploys. Activate when the user wants to manage sites and deploys via Netlify, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Netlify** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Netlify web hosting platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Netlify web hosting platform using the Gumloop MCP connection to `netlify`.
...
current = safe_call(client, 'netlify', 'list_sites', 'list_sites', {})
...
result = safe_call(client, 'netlify', 'list_sites', 'list_sites', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'netlify', 'create_deploy', 'create_deploy', {'site_id': 'site_abc', 'files': {'index.html': '<h1>Hello</h1>'}})
...
result = safe_call(client, 'netlify', 'update_site', 'update_site', {'site_id': 'site_abc', 'name': 'my-new-site'})
...
result = safe_call(client, 'netlify', 'delete_site', 'delete_site', {'site_id': 'site_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `netlify` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.netlify.com/
- Tool discovery: Use `safe_call(client, 'netlify', 'tool_discovery', {})` to list available tools at runtime.
