...
color: Blue
---
# Gbigquery MCP Integration

## Overview
This skill covers working with the **Gbigquery** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google BigQuery data warehouse operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google BigQuery data warehouse using the Gumloop MCP connection to `gbigquery`.
...
current = safe_call(client, 'gbigquery', 'run_query', 'run_query', {'query': 'SELECT * FROM `project.dataset.table` LIMIT 10'})
...
result = safe_call(client, 'gbigquery', 'run_query', 'run_query', {'query': 'SELECT * FROM `project.dataset.table` LIMIT 10'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gbigquery', 'run_query', 'run_query', {'query': 'INSERT INTO `project.dataset.table` (id, name) VALUES (1, "test")'})
...
result = safe_call(client, 'gbigquery', 'run_query', 'run_query', {'query': 'UPDATE `project.dataset.table` SET name = "updated" WHERE id = 1'})
...
result = safe_call(client, 'gbigquery', 'run_query', 'run_query', {'query': 'DELETE FROM `project.dataset.table` WHERE id = 1'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gbigquery` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://cloud.google.com/bigquery/docs/reference/standard-sql
- Tool discovery: Use `safe_call(client, 'gbigquery', 'tool_discovery', {})` to list available tools at runtime.
