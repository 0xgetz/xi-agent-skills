...
## Overview
This skill covers working with the **Redis** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Redis key-value store operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Redis key-value store using the Gumloop MCP connection to `redis`.
...
current = safe_call(client, 'redis', 'get', 'get', {'key': 'user:123:session'})
...
result = safe_call(client, 'redis', 'get', 'get', {'key': 'user:123:session'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'redis', 'set', 'set', {'key': 'user:123:session', 'value': 'token_abc', 'ttl': 3600})
...
result = safe_call(client, 'redis', 'set', 'set', {'key': 'config:theme', 'value': 'dark'})
...
result = safe_call(client, 'redis', 'delete', 'delete', {'keys': ['cache:temp_data', 'session:expired']})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `redis` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://redis.io/docs/
- Tool discovery: Use `safe_call(client, 'redis', 'tool_discovery', {})` to list available tools at runtime.
