...
## Overview
This skill covers working with the **Kubernetes** integration over the Model Context Protocol (MCP) to inspect and manage Kubernetes resources.

## When to use this skill
Activate when the user wants to inspect and manage Kubernetes resources using Kubernetes, or asks to connect, automate, or query Kubernetes.

## Workflow
1. **Discover** the available Kubernetes tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("kubernetes", "<tool_name>", { }).results[0]
