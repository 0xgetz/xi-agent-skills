...
## Overview
This skill covers working with the **Calendly** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Calendly scheduling platform operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Calendly scheduling platform using the Gumloop MCP connection to `calendly`.
...
current = safe_call(client, 'calendly', 'list_events', 'list_events', {'user': 'https://api.calendly.com/users/U123', 'count': 10})
...
result = safe_call(client, 'calendly', 'list_events', 'list_events', {'user': 'https://api.calendly.com/users/U123', 'count': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'calendly', 'create_scheduling_link', 'create_scheduling_link', {'owner': 'https://api.calendly.com/event_types/ET123', 'max_event_count': 1})
...
result = safe_call(client, 'calendly', 'cancel_event', 'cancel_event', {'event_uuid': 'abc123', 'reason': 'Scheduling conflict'})
...
result = safe_call(client, 'calendly', 'cancel_event', 'cancel_event', {'event_uuid': 'abc123', 'reason': 'User requested cancellation'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `calendly` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.calendly.com/
- Tool discovery: Use `safe_call(client, 'calendly', 'tool_discovery', {})` to list available tools at runtime.
