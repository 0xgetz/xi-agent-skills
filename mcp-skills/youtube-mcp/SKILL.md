---
name: youtube-mcp
description: Use the YouTube MCP integration to search videos and read channel data. Activate when the user wants to search videos and read channel data via YouTube, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Youtube** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all YouTube video management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage YouTube video management using the Gumloop MCP connection to `youtube`.
...
current = safe_call(client, 'youtube', 'search_videos', 'search_videos', {'query': 'MCP tutorial', 'max_results': 10})
...
result = safe_call(client, 'youtube', 'search_videos', 'search_videos', {'query': 'MCP tutorial', 'max_results': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'youtube', 'upload_video', 'upload_video', {'title': 'MCP Integration Guide', 'description': 'Learn to use MCP with Gumloop', 'privacy_status': 'public'})
...
result = safe_call(client, 'youtube', 'update_video', 'update_video', {'video_id': 'vid_abc', 'title': 'Updated: MCP Guide v2'})
...
result = safe_call(client, 'youtube', 'delete_video', 'delete_video', {'video_id': 'vid_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `youtube` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/youtube/v3/
- Tool discovery: Use `safe_call(client, 'youtube', 'tool_discovery', {})` to list available tools at runtime.
