...
## Overview
This skill covers working with the **Xero** integration over the Model Context Protocol (MCP) to manage accounting records and invoices.

## When to use this skill
Activate when the user wants to manage accounting records and invoices using Xero, or asks to connect, automate, or query Xero.

## Workflow
1. **Discover** the available Xero tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("xero", "<tool_name>", { }).results[0]
