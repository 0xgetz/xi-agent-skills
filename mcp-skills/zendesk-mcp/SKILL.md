---
name: zendesk-mcp
description: Use the Zendesk MCP integration to read and manage support tickets. Activate when the user wants to read and manage support tickets via Zendesk, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Zendesk** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Zendesk customer support operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Zendesk customer support using the Gumloop MCP connection to `zendesk`.
...
current = safe_call(client, 'zendesk', 'list_tickets', 'list_tickets', {'query': 'status:open', 'limit': 20})
...
result = safe_call(client, 'zendesk', 'list_tickets', 'list_tickets', {'query': 'status:open', 'limit': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'zendesk', 'create_ticket', 'create_ticket', {'subject': 'Support needed', 'comment': {'body': 'Please help with...'}, 'priority': 'normal'})
...
result = safe_call(client, 'zendesk', 'update_ticket', 'update_ticket', {'ticket_id': 12345, 'status': 'solved', 'assignee_id': 67890})
...
result = safe_call(client, 'zendesk', 'delete_ticket', 'delete_ticket', {'ticket_id': 99999})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `zendesk` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.zendesk.com/api-reference/
- Tool discovery: Use `safe_call(client, 'zendesk', 'tool_discovery', {})` to list available tools at runtime.
