---
name: notion-mcp
description: Use the Notion MCP integration to query databases and manage pages. Activate when the user wants to query databases and manage pages via Notion, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Notion** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Notion workspaces operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Notion workspaces using the Gumloop MCP connection to `notion`.
...
current = safe_call(client, 'notion', 'query_database', 'query_database', {'database_id': 'db_abc123', 'filter': {'property': 'Status', 'select': {'equals': 'In Progress'}}})
...
result = safe_call(client, 'notion', 'query_database', 'query_database', {'database_id': 'db_abc123', 'filter': {'property': 'Status', 'select': {'equals': 'In Progress'}}})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'notion', 'create_page', 'create_page', {'parent': {'database_id': 'db_abc123'}, 'properties': {'Name': {'title': [{'text': {'content': 'New Task'}}]}, 'Status': {'select': {'name': 'To Do'}}}})
...
result = safe_call(client, 'notion', 'update_page', 'update_page', {'page_id': 'pg_abc123', 'properties': {'Status': {'select': {'name': 'Done'}}}})
...
result = safe_call(client, 'notion', 'archive_page', 'archive_page', {'page_id': 'pg_xyz789'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `notion` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.notion.com/
- Tool discovery: Use `safe_call(client, 'notion', 'tool_discovery', {})` to list available tools at runtime.
