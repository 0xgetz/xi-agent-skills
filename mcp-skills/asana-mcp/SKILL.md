---
name: asana-mcp
description: Use the Asana MCP integration to manage Asana tasks and projects. Activate when the user wants to manage Asana tasks and projects via Asana, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Asana** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Asana project management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Asana project management using the Gumloop MCP connection to `asana`.
...
current = safe_call(client, 'asana', 'get_task', 'get_task', {'task_gid': '123456'})
...
result = safe_call(client, 'asana', 'get_task', 'get_task', {'task_gid': '123456'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'asana', 'create_task', 'create_task', {'project_gid': 'proj_abc', 'name': 'Design new landing page', 'notes': 'Figma link in brief', 'assignee': 'user_abc'})
...
result = safe_call(client, 'asana', 'update_task', 'update_task', {'task_gid': '123456', 'completed': True, 'notes': 'Updated notes'})
...
result = safe_call(client, 'asana', 'delete_task', 'delete_task', {'task_gid': '999999'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `asana` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.asana.com/
- Tool discovery: Use `safe_call(client, 'asana', 'tool_discovery', {})` to list available tools at runtime.
