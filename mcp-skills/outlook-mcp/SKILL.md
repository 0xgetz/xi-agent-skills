---
name: outlook-mcp
description: Use the Outlook MCP integration to read and send Outlook email. Activate when the user wants to read and send Outlook email via Outlook, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Outlook** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Microsoft Outlook / Exchange email operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Microsoft Outlook / Exchange email using the Gumloop MCP connection to `outlook`.
...
current = safe_call(client, 'outlook', 'list_emails', 'list_emails', {'folder': 'Inbox', 'top': 10})
...
result = safe_call(client, 'outlook', 'list_emails', 'list_emails', {'folder': 'Inbox', 'top': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'outlook', 'send_email', 'send_email', {'to': ['recipient@example.com'], 'subject': 'Meeting notes', 'body': 'Notes from today'})
...
result = safe_call(client, 'outlook', 'mark_as_read', 'mark_as_read', {'message_id': 'msg_abc'})
...
result = safe_call(client, 'outlook', 'delete_message', 'delete_message', {'message_id': 'msg_abc'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `outlook` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://learn.microsoft.com/en-us/graph/api/resources/mail-api-overview
- Tool discovery: Use `safe_call(client, 'outlook', 'tool_discovery', {})` to list available tools at runtime.
