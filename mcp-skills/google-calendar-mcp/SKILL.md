...
description: Use the Google Calendar MCP integration to read and manage calendar events. Activate when the user wants to read and manage calendar events via Google Calendar, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
# Gcalendar MCP Integration

## Overview
This skill covers working with the **Gcalendar** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Google Calendar scheduling operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Google Calendar scheduling using the Gumloop MCP connection to `gcalendar`.
...
current = safe_call(client, 'gcalendar', 'list_events', 'list_events', {'calendar_id': 'primary', 'time_min': '2025-01-01T00:00:00Z', 'time_max': '2025-01-31T23:59:59Z'})
...
result = safe_call(client, 'gcalendar', 'list_events', 'list_events', {'calendar_id': 'primary', 'time_min': '2025-01-01T00:00:00Z', 'time_max': '2025-01-31T23:59:59Z'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'gcalendar', 'create_event', 'create_event', {'calendar_id': 'primary', 'summary': 'Sprint Planning', 'start': {'dateTime': '2025-01-15T10:00:00', 'timeZone': 'America/New_York'}, 'end': {'dateTime': '2025-01-15T11:00:00', 'timeZone': 'America/New_York'}})
...
result = safe_call(client, 'gcalendar', 'update_event', 'update_event', {'calendar_id': 'primary', 'event_id': 'evt_abc', 'summary': 'Rescheduled: Sprint Planning'})
...
result = safe_call(client, 'gcalendar', 'delete_event', 'delete_event', {'calendar_id': 'primary', 'event_id': 'evt_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `gcalendar` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.google.com/calendar/api/
- Tool discovery: Use `safe_call(client, 'gcalendar', 'tool_discovery', {})` to list available tools at runtime.
