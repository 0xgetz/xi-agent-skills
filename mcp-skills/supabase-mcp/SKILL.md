---
name: supabase-mcp
description: Use the Supabase MCP integration to query Supabase Postgres and storage. Activate when the user wants to query Supabase Postgres and storage via Supabase, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Supabase** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Supabase backend-as-a-service operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Supabase backend-as-a-service using the Gumloop MCP connection to `supabase`.
...
current = safe_call(client, 'supabase', 'select', 'select', {'table': 'profiles', 'columns': 'id, name, email', 'limit': 10})
...
result = safe_call(client, 'supabase', 'select', 'select', {'table': 'profiles', 'columns': 'id, name, email', 'limit': 10})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'supabase', 'insert', 'insert', {'table': 'profiles', 'values': {'name': 'Dave', 'email': 'dave@example.com'}})
...
result = safe_call(client, 'supabase', 'update', 'update', {'table': 'profiles', 'values': {'avatar_url': 'new.jpg'}, 'eq': {'id': 42}})
...
result = safe_call(client, 'supabase', 'delete', 'delete', {'table': 'profiles', 'eq': {'status': 'inactive'}})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `supabase` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://supabase.com/docs
- Tool discovery: Use `safe_call(client, 'supabase', 'tool_discovery', {})` to list available tools at runtime.
