---
name: cloudflare-mcp
description: Use the Cloudflare MCP integration to manage DNS, workers, and caching. Activate when the user wants to manage DNS, workers, and caching via Cloudflare, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Cloudflare** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Cloudflare DNS and CDN operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Cloudflare DNS and CDN using the Gumloop MCP connection to `cloudflare`.
...
current = safe_call(client, 'cloudflare', 'list_zones', 'list_zones', {})
...
result = safe_call(client, 'cloudflare', 'list_zones', 'list_zones', {})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'cloudflare', 'purge_cache', 'purge_cache', {'zone_id': 'zone123', 'files': ['https://example.com/styles.css']})
...
result = safe_call(client, 'cloudflare', 'update_dns_record', 'update_dns_record', {'zone_id': 'zone123', 'record_id': 'rec456', 'content': '192.168.1.1'})
...
result = safe_call(client, 'cloudflare', 'delete_dns_record', 'delete_dns_record', {'zone_id': 'zone123', 'record_id': 'rec789'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `cloudflare` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developers.cloudflare.com/api/
- Tool discovery: Use `safe_call(client, 'cloudflare', 'tool_discovery', {})` to list available tools at runtime.
