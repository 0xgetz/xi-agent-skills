...
## Overview
This skill covers working with the **Confluence** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Confluence knowledge management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Confluence knowledge management using the Gumloop MCP connection to `confluence`.
...
current = safe_call(client, 'confluence', 'get_page', 'get_page', {'page_id': '123456'})
...
result = safe_call(client, 'confluence', 'get_page', 'get_page', {'page_id': '123456'})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'confluence', 'create_page', 'create_page', {'space_key': 'ENG', 'title': 'Sprint Notes v42', 'body': {'storage': {'value': '<p>Sprint retrospective notes</p>', 'representation': 'storage'}}})
...
result = safe_call(client, 'confluence', 'update_page', 'update_page', {'page_id': '123456', 'title': 'Updated Sprint Notes', 'body': {'storage': {'value': '<p>Updated content</p>', 'representation': 'storage'}}})
...
result = safe_call(client, 'confluence', 'delete_page', 'delete_page', {'page_id': '999999'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `confluence` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.atlassian.com/cloud/confluence/rest/
- Tool discovery: Use `safe_call(client, 'confluence', 'tool_discovery', {})` to list available tools at runtime.
