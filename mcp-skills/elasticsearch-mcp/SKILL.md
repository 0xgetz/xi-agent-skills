...
## Overview
This skill covers working with the **Elasticsearch** integration over the Model Context Protocol (MCP) to search and index documents.

## When to use this skill
Activate when the user wants to search and index documents using Elasticsearch, or asks to connect, automate, or query Elasticsearch.

## Workflow
1. **Discover** the available Elasticsearch tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("elasticsearch", "<tool_name>", { }).results[0]
