...
## Overview
This skill covers working with the **AWS S3** integration over the Model Context Protocol (MCP) to read and write objects in S3 buckets.

## When to use this skill
Activate when the user wants to read and write objects in S3 buckets using AWS S3, or asks to connect, automate, or query AWS S3.

## Workflow
1. **Discover** the available AWS S3 tools via tool discovery before calling anything — never assume a tool exists.
...
client = Gumloop()
res = client.mcp.execute("aws-s3", "<tool_name>", { }).results[0]
