...
## Overview
This skill covers working with the **Box** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Box cloud content management operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Box cloud content management using the Gumloop MCP connection to `box`.
...
current = safe_call(client, 'box', 'list_folder_items', 'list_folder_items', {'folder_id': '0', 'limit': 20})
...
result = safe_call(client, 'box', 'list_folder_items', 'list_folder_items', {'folder_id': '0', 'limit': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'box', 'upload_file', 'upload_file', {'folder_id': '12345', 'name': 'report.pdf', 'content': '...'})
...
result = safe_call(client, 'box', 'update_file', 'update_file', {'file_id': 'file_abc', 'name': 'renamed_report.pdf'})
...
result = safe_call(client, 'box', 'delete_file', 'delete_file', {'file_id': 'file_xyz'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `box` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.box.com/
- Tool discovery: Use `safe_call(client, 'box', 'tool_discovery', {})` to list available tools at runtime.
