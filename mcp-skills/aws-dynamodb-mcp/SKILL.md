...
description: Use the AWS DynamoDB MCP integration to query DynamoDB tables. Activate when the user wants to query DynamoDB tables via AWS DynamoDB, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
# Aws_Dynamodb MCP Integration

## Overview
This skill covers working with the **Aws_Dynamodb** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all AWS DynamoDB NoSQL database operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage AWS DynamoDB NoSQL database using the Gumloop MCP connection to `aws_dynamodb`.
...
current = safe_call(client, 'aws_dynamodb', 'get_item', 'get_item', {'table': 'Users', 'key': {'user_id': 'u123'}})
...
result = safe_call(client, 'aws_dynamodb', 'get_item', 'get_item', {'table': 'Users', 'key': {'user_id': 'u123'}})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'aws_dynamodb', 'put_item', 'put_item', {'table': 'Users', 'item': {'user_id': 'u456', 'name': 'Frank', 'created_at': '2025-01-01'}})
...
result = safe_call(client, 'aws_dynamodb', 'update_item', 'update_item', {'table': 'Users', 'key': {'user_id': 'u123'}, 'update_expression': 'SET status = :s', 'expression_attribute_values': {':s': 'active'}})
...
result = safe_call(client, 'aws_dynamodb', 'delete_item', 'delete_item', {'table': 'Users', 'key': {'user_id': 'u999'}})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `aws_dynamodb` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://docs.aws.amazon.com/dynamodb/
- Tool discovery: Use `safe_call(client, 'aws_dynamodb', 'tool_discovery', {})` to list available tools at runtime.
