---
name: twilio-mcp
description: Use the Twilio MCP integration to send SMS and manage calls. Activate when the user wants to send SMS and manage calls via Twilio, including discovering the right tool, building parameters, and handling results.
icon: plug
color: Blue
---
...
## Overview
This skill covers working with the **Twilio** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Twilio SMS and voice API operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Twilio SMS and voice API using the Gumloop MCP connection to `twilio`.
...
current = safe_call(client, 'twilio', 'list_messages', 'list_messages', {'limit': 20})
...
result = safe_call(client, 'twilio', 'list_messages', 'list_messages', {'limit': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'twilio', 'send_sms', 'send_sms', {'to': '+1234567890', 'from': '+0987654321', 'body': 'Your verification code is 123456'})
...
result = safe_call(client, 'twilio', 'update_message', 'update_message', {'message_sid': 'SMabc123', 'body': 'Updated text'})
...
result = safe_call(client, 'twilio', 'delete_message', 'delete_message', {'message_sid': 'SMabc123'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `twilio` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://www.twilio.com/docs
- Tool discovery: Use `safe_call(client, 'twilio', 'tool_discovery', {})` to list available tools at runtime.
