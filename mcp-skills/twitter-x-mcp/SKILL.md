...
color: Blue
---
# Twitter_X MCP Integration

## Overview
This skill covers working with the **Twitter_X** integration via the MCPClient from `lib.gumloop_mcp`. The MCPClient wraps the Gumloop MCP transport layer with automatic retries, error handling, and typed responses. Use it for all Twitter/X social media operations.

## When to use this skill
Activate when the user wants to query, create, update, or manage Twitter/X social media using the Gumloop MCP connection to `twitter_x`.
...
current = safe_call(client, 'twitter_x', 'search_tweets', 'search_tweets', {'query': 'from:@gumloop', 'max_results': 20})
...
result = safe_call(client, 'twitter_x', 'search_tweets', 'search_tweets', {'query': 'from:@gumloop', 'max_results': 20})
print('Result:', result)
```
...
# Now create/update
result = safe_call(client, 'twitter_x', 'create_tweet', 'create_tweet', {'text': 'Excited to announce our latest feature!'})
...
result = safe_call(client, 'twitter_x', 'create_tweet', 'create_tweet', {'text': 'Updated announcement'})
...
result = safe_call(client, 'twitter_x', 'delete_tweet', 'delete_tweet', {'tweet_id': '1234567890'})
print('Delete completed:', result)
```

## Artificial API / REST Fallback
If the native Gumloop `twitter_x` connection is unavailable, fall back to the raw REST API:
```python
import os
import requests
...
- Native API reference: https://developer.x.com/en/docs
- Tool discovery: Use `safe_call(client, 'twitter_x', 'tool_discovery', {})` to list available tools at runtime.
