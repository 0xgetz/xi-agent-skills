...
## Overview
Aggregates trending token data on Base and sends a periodic digest.
...
```python
def fetch_trending():
    pairs = [p for p in requests.get(f"https://api.dexscreener.com/token-pairs/v1/8453", timeout=15).json()
...
def send_digest():
    top = fetch_trending()
    lines = [f"📈 *Trending on Base — {datetime.now().strftime('%H:%M UTC')}*\n"]
...
```
```bash
docker build -t tg-base-trending .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e TOP_TRENDING=15 tg-base-trending
```
