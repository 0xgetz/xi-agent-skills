...
## Overview
Checks token sellability on Base via eth_call simulation.
...
```
```bash
docker build -t tg-base-honeypot .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-base-honeypot
```
