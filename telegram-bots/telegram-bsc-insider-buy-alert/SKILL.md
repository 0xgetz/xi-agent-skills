...
## Overview
Flags wallets on BSC that buy tokens before public announcements.
...
```
```bash
docker build -t tg-bsc-insider .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-bsc-insider
```
