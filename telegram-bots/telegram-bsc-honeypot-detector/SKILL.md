...
## Overview
Checks token sellability on BSC via eth_call simulation.
...
```
```bash
docker build -t tg-bsc-honeypot .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-bsc-honeypot
```
