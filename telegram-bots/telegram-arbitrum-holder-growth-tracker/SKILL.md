...
## Overview
Tracks holder count changes for tokens on Arbitrum.
...
def check():
    for t in WATCH:
        ...
        if abs(pct) >= 5:
            em = "📈" if c > p else "📉"
            send_alert(config, f"{em} *Holder Change on Arbitrum*\n`{escape_md(t[:10])}...`\n{p} → {c} ({pct:+.1f}%)")
        prevs[t.strip()] = c
```
...
```
```bash
docker build -t tg-arb-holders .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_TOKENS=0xToken1,0xToken2 tg-arb-holders
```
