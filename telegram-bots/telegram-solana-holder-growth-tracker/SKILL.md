...
## Overview
Tracks holder count changes for tokens on Solana.
...
def check():
    for t in WATCH:
        ...
        if abs(pct) >= 5:
            em = "📈" if c > p else "📉"
            send_alert(config, f"{em} *Holder Change on Solana*\n`{escape_md(t[:10])}...`\n{p} → {c} ({pct:+.1f}%)")
        prevs[t.strip()] = c
```
...
```
```bash
docker build -t tg-sol-holders .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_TOKENS=0xToken1,0xToken2 tg-sol-holders
```
