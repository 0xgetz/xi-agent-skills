...
## Overview
Monitors large wallet addresses on Polygon for significant token transfers.
...
```
```bash
docker build -t tg-poly-whale .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WHALE_WALLETS=0xAddr1,0xAddr2 tg-poly-whale
```
