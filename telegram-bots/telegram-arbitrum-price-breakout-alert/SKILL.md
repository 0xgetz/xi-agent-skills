...
## Overview
Detects price breakouts from consolidation ranges on Arbitrum.
...
def detect(t):
    ...
    if abs(z) >= STD:
        ...
        send_alert(config, f"{d} *Breakout on Arbitrum*\n`{escape_md(t[:10])}...`\nPrice: ${p:.8f}\nZ-score: {z:.2f}")
```
...
```
```bash
docker build -t tg-arb-breakout .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_TOKENS=0xToken -e BREAKOUT_Z=2.5 tg-arb-breakout
```
