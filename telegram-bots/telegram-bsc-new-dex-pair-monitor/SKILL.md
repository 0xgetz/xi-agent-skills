...
## Overview
Monitors factory contracts for newly created trading pairs on BSC.
...
```
```bash
docker build -t tg-bsc-newpairs .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-bsc-newpairs
```
