...
## Overview
Checks token sellability on Polygon via eth_call simulation.
...
```
```bash
docker build -t tg-poly-honeypot .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-poly-honeypot
```
