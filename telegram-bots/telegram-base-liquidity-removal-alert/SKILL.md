...
## Overview
Monitors DEX pools on Base for LP removal events — a classic rug-pull precursor.
...
```
```bash
docker build -t tg-base-liq-removal .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_PAIRS=0xPair1,0xPair2 tg-base-liq-removal
```
