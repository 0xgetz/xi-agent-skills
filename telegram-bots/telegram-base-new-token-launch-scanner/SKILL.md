...
## Overview
Detects newly deployed token contracts on Base, evaluates risk, and sends alerts.
...
def fetch_new_tokens():
    url = f"https://api.dexscreener.com/token-pairs/v1/8453"
...
```
```bash
docker build -t tg-base-newtoken .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-base-newtoken
```
