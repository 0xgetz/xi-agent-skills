---
name: telegram-base-new-token-launch-scanner
description: Build a Telegram bot to detect new token deployments on Base, with on-chain/market data sourcing, risk/honeypot filtering, and Telegram alert delivery. Activate when the user wants to detect new token deployments on Base and get alerts in Telegram.
icon: send
color: Teal
---

# Telegram Base New Token Launch Scanner

## Overview
A Telegram bot skill that helps detect new token deployments on Base. It is a research/monitoring tool that pushes timely alerts to a Telegram chat or channel — it does **not** guarantee profit and does not place trades for you unless you explicitly wire that in.

## When to use this skill
Activate when the user wants to detect new token deployments on Base and receive alerts in Telegram.
