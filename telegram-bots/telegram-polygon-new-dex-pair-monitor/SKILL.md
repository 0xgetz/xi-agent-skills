...
## Overview
Monitors factory contracts for newly created trading pairs on Polygon.
...
```
```bash
docker build -t tg-poly-newpairs .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-poly-newpairs
```
