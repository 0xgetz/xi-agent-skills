...
## Overview
Surfaces potential airdrop opportunities on Solana by detecting high-activity, low-liquidity tokens.
...
```python
def scan():
    pairs = requests.get(f"https://api.dexscreener.com/token-pairs/v1/0", timeout=15).json()
    for p in pairs:
        txs = int(p.get("txns", {"h24": 0})["h24"])
        liq = float(p.get("liquidity", {"usd": 0})["usd"])
        if txs > 500 and liq < 5000:
            msg = (
                f"🎁 *Airdrop Candidate on Solana*\n"
...
```
```bash
docker build -t tg-sol-airdrop .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-sol-airdrop
```
