...
## Overview
Checks token sellability on Solana via eth_call simulation.
...
```
```bash
docker build -t tg-sol-honeypot .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-sol-honeypot
```
