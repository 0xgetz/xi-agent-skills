...
## Overview
Monitors large wallet addresses on Ethereum for significant token transfers.
...
```
```bash
docker build -t tg-eth-whale .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WHALE_WALLETS=0xAddr1,0xAddr2 tg-eth-whale
```
