---
name: telegram-honeypot-detector
description: Build a Telegram bot to check if a token can be sold before buying, with on-chain/market data sourcing, risk/honeypot filtering, and Telegram alert delivery. Activate when the user wants to check if a token can be sold before buying and get alerts in Telegram.
icon: send
color: Teal
---

# Telegram Honeypot Detector

## Overview
A Telegram bot skill that helps check if a token can be sold before buying. It is a research/monitoring tool that pushes timely alerts to a Telegram chat or channel — it does **not** guarantee profit and does not place trades for you unless you explicitly wire that in.

## When to use this skill
Activate when the user wants to check if a token can be sold before buying and receive alerts in Telegram.
