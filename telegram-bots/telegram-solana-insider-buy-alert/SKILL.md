...
## Overview
Flags wallets on Solana that buy tokens before public announcements.
...
```
```bash
docker build -t tg-sol-insider .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-sol-insider
```
