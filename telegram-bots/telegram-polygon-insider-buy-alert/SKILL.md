...
## Overview
Flags wallets on Polygon that buy tokens before public announcements.
...
```
```bash
docker build -t tg-poly-insider .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-poly-insider
```
