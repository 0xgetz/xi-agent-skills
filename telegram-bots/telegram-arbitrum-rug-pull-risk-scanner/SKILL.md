...
## Overview
Scores token contracts on Arbitrum for rug-pull indicators (mint, blacklist, proxy, tax).
...
def score(tok):
    ...
    s = min(s, 10)
    send_alert(config, f"{em} *Rug Scan on Arbitrum*\nScore: {s}/10\n{'; '.join(r) if r else 'No issues found'}")
```
...
```
```bash
docker build -t tg-arb-rug .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-arb-rug
```

## Production Deployment
| Platform | Notes |
|----------|-------|
| Railway | Add ARB_ETHERSCAN_KEY as env var |
