---
name: telegram-polygon-volume-spike-alert
description: Build a Telegram bot to alert on volume surges on Polygon, with on-chain/market data sourcing, risk/honeypot filtering, and Telegram alert delivery. Activate when the user wants to alert on volume surges on Polygon and get alerts in Telegram.
icon: send
color: Teal
---
# Telegram Polygon Volume Spike Alert

## Overview
Detects abnormal DEX volume spikes compared to trailing averages on Polygon.
...
def vol(tok):
    ...
    return sum(float(x.get("volume", {"h24": 0})["h24"]) for x in p if x.get("chainId") == "poly") or 0
...
def check():
    for t in WATCH:
        ...
        ratio = h[-1] / avg
        if ratio >= MULT:
            send_alert(config, f"📊 *Volume Spike on Polygon*\n`{escape_md(t[:10])}...`\n{ratio:.1f}x baseline\nVol: ${v:,.0f}")
```

## Subgraph Query (DEX Analytics)
```python
# Example: query Uniswap subgraph for pool volume
SUBGRAPH = "https://api.thegraph.com/subgraphs/name/uniswap/uniswap-v3-poly"
...
```
```bash
docker build -t tg-poly-vol .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_TOKENS=0xToken tg-poly-vol
```
