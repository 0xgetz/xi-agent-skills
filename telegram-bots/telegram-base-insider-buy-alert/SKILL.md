...
## Overview
Flags wallets on Base that buy tokens before public announcements.
...
```
```bash
docker build -t tg-base-insider .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-base-insider
```
