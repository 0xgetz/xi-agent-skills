...
## Overview
Monitors DEX pools on Arbitrum for LP removal events — a classic rug-pull precursor.
...
def monitor():
    for pair in WATCH:
        ...
        if p and cur < p:
            pct = (p - cur) / p * 100
            msg = (
                f"⚠️ *Liquidity Removed on Arbitrum*\n"
                f"Pair: `{pair[:10]}...`\n"
                f"Removed: {pct:.1f}%\n"
...
```
```bash
docker build -t tg-arb-liq-removal .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y -e WATCH_PAIRS=0xPair1,0xPair2 tg-arb-liq-removal
```
