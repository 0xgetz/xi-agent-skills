...
## Overview
Monitors factory contracts for newly created trading pairs on Arbitrum.
...
def monitor():
    ...
    for log in logs:
        ...
        msg = f"🆕 *New DEX Pair on Arbitrum*\nPair: `{pair[:10]}...`\nToken0: `{t0[:10]}...`\nToken1: `{t1[:10]}...`"
...
```
```bash
docker build -t tg-arb-newpairs .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-arb-newpairs
```
