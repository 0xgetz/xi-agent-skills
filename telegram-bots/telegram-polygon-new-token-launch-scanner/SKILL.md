---
name: telegram-polygon-new-token-launch-scanner
description: Build a Telegram bot to detect new token deployments on Polygon, with on-chain/market data sourcing, risk/honeypot filtering, and Telegram alert delivery. Activate when the user wants to detect new token deployments on Polygon and get alerts in Telegram.
icon: send
color: Teal
---

# Telegram Polygon New Token Launch Scanner

## Overview
A Telegram bot skill that helps detect new token deployments on Polygon. It is a research/monitoring tool that pushes timely alerts to a Telegram chat or channel — it does **not** guarantee profit and does not place trades for you unless you explicitly wire that in.

## When to use this skill
Activate when the user wants to detect new token deployments on Polygon and receive alerts in Telegram.
