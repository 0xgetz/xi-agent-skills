...
## Overview
Detects newly deployed token contracts on Polygon, evaluates risk, and sends alerts.
...
def fetch_new_tokens():
    url = f"https://api.dexscreener.com/token-pairs/v1/137"
...
```
```bash
docker build -t tg-poly-newtoken .
docker run -d -e TELEGRAM_BOT_TOKEN=x -e TELEGRAM_CHAT_ID=y tg-poly-newtoken
```
